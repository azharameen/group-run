...
- AD-19 (work-item status lifecycle) adopted 2026-08-17: Status vocabulary = Backlog / In Progress / In Review / On Hold / Done (owner renamed Todo to Backlog, added In Review + On Hold). Story/Task/Bug: Backlog -> In Progress (dev start) -> In Review (PR) -> Done (merged); Epic: Backlog (planning) -> In Progress (first story starts) -> Done (all children Done + retro complete); deferrals = On Hold + ledger entry. Board corrected same day: open epics #29-#33 moved In Progress -> Backlog (no story had started).
- BMAD team overrides created via the bmad-customize mechanism in _bmad/custom/ (no direct skill edits): bmad-sprint-planning, bmad-create-epics-and-stories, bmad-create-story, bmad-dev-story (persistent_facts + on_complete GH-SYNC hooks appended alongside the existing retro action-item gates) and bmad-retrospective (new). Verified through resolve_customization.py (all five merge cleanly). Board field/option/iteration IDs + item map + sync procedure reference: _bmad-output/implementation-artifacts/github-board.md, loaded as a persistent fact by all five skills.
- 2026-08-17 Jules audit: PR #18 (e2e isolation, #9) merged with a stale worktree - commit 6aa2566 deleted docs/performance-testing.md + docs/sse-streaming.md (PR #13/#15 work), reverted both Dockerfiles (PR #16) and the ci.yml main-branch e2e trigger (PR #14), and weakened the chat.spec.ts stop-generation test (no longer clicks stop). Issues #8, #9, #10, #11, #12 REOPENED with restoration instructions + acceptance criteria (fresh branch from latest develop, diff-scoped). Board: all 5 items Done -> Backlog (verified). The #9 isolation mechanism itself (POST /api/testing/reset incl. interrupts table, autoResetState fixture, workers:1) is approved and stays. OPEN: Jules restoration PRs for #8/#9/#10/#11/#12; production changes in useChatStream.ts/CommandCenter.tsx from PR #18 need justification in the #9 follow-up PR.
- 2026-08-17 RESOLVED - Jules did not pick up the reopened comments; Ameen decided the project session adapts the changes here. Commit 6397589 on feat/project-management-discussions (pushed) restores: perf docs + docs/README link (from f6ed4ff), SSE docs + README links (from 1f7fbbd), both Dockerfiles (from 92f4e92), ci.yml main-branch e2e trigger (from e55770c), and the chat.spec.ts stop-generation test (original assertions, PR #18 test-1 improvements kept). Verified byte-identical to the original commits. useChatStream.ts prevThreadIdRef guard + CommandCenter.tsx ensureThread refactor KEPT with justification (thread-switch abort leak fix; behavior-preserving refactor). Issues #8, #9, #10, #11, #12 commented + CLOSED. Board: 5 items Backlog -> Done (verified via GraphQL). New rule AD-20 recorded in spine + this file: Jules delegation ONE AT A TIME (no bulk), Jules-bound issue titles carry suffix " - jules", agent branches fresh from latest develop + diff-scoped before merge.

## Deferred from: dev of 8-1-create-and-initialize-an-organization-structure (2026-08-17)

- Org dashboard Playwright e2e coverage — deferred per story spec Dev Notes (e2e line): story 8.1 shipped with unit/component tests only (16 backend + 5 Vitest). Original blocker #9 (e2e data isolation) was still open at story-creation time. **SUPERSEDED 2026-08-17:** #9 is now CLOSED (isolation mechanism in develop via PR #18 + restoration 6397589), so the follow-up is schedulable — tracked as **issue #54** (sub-issue of story #35, board: Task / Backlog, item `g21E5w`). This ledger line closes when #54 lands.

## Deferred from: code review of 8-1-create-and-initialize-an-organization-structure.md (2026-08-17)

- W1: Blocking SQLite I/O in `async def` routes blocks the event loop (`backend/app/api/routes/organizations.py` — all three endpoints; org create performs up to 24 sqlite round-trips on the loop thread). Pre-existing codebase pattern: `ideas.py`, `threads.py`, `knowledge_base.py` use the same `async def` + direct sync sqlite idiom the story docstring explicitly mirrors. Fixing one route file in isolation breaks consistency — needs a codebase-wide decision (runpool/`anyio.to_thread` or sync `def` routes) as a separate story.
- W2: Raw JSON in user-facing error messages (`frontend/src/api/organizations.ts:10` — `throw new Error(\`API ${res.status}: ${text}\`)` renders raw JSON bodies in the Organization error card/toasts). Pre-existing convention, byte-identical to `ideas.ts`/`knowledge.ts`. Fix belongs to a shared API-error-formatting pass across all api modules.
- W3: No fetch timeout/abort on the organization API client (`frontend/src/api/organizations.ts` — hung request leaves the loading skeleton indefinitely). Pre-existing pattern: no API module uses `AbortController` or a timeout. Interaction with W1 makes a stall more likely on this page (largest sync I/O in an async route); fix alongside W1/W2 in the shared pass.
- D1 (decision): Organization selection/switching in the UI — the dashboard renders `organizations[0]` (most recent) and there is no way to view earlier orgs (no DELETE by design, so orgs accumulate). Accepted for story 8.1: its ACs only require viewing one populated org. Follow-up story: org selector over `GET /api/organizations` (dropdown in the Command Center organization page).

## Deferred from: code review of spec-8-2-submit-and-route-work-item.md (2026-08-18)

- source_spec: `_bmad-output/implementation-artifacts/spec-8-2-submit-and-route-work-item.md`
  summary: DOMAIN_TOOLS wiring and CoS intake instructions are not scoped to the CoS/general team, so every future non-CoS team inherits the submit_work_item tool and the intake prompt section.
  evidence: `runtime.py` appends `DOMAIN_TOOLS` to every agent team's tool list and `instructions/global-agent-instructions.md` is shared by all teams; currently only the `general` (CoS) team exists so the leak is latent, and the spec's Block-If list forbids touching teams.yaml, so team-scoping needs a deliberate follow-up decision (per-team tool filter or team-specific instruction file).
- source_spec: `_bmad-output/implementation-artifacts/spec-8-2-submit-and-route-work-item.md`
  summary: `repository._routing_map` builds an `IN (...)` placeholder list from the requested work-item ids, which exceeds SQLite's variable limit at ~999 items and 500s the list endpoint for a single very large org.
  evidence: `list_work_items_with_routing` passes the full id list from one page of items into `_routing_map`; at 1000+ work items in one org the `IN` clause holds more variables than SQLite's compile-time limit (default 999). Unreachable at v1 scale; fix by chunking the id list into batches of ~500 when implementing list pagination (Epic 12) or moving to a JOIN.

  ## E2E strategy adoption (2026-08-18)

  - AD-21 adopted in the spine (owner decision 2026-08-18, party round-table): keep Playwright + in-memory SQLite + mock LLM (no Postgres; revisit triggers: suite >10 min in CI, CI sharding, prod RDBMS at Epic 12); coverage model = per-story primary-flow e2e case as a standing AC line + one spec per stable feature area (epic close requires feature-area golden path); reliability = hardening + 3x back-to-back green proof as a single Jules task; quarantine = skip + tracked issue, never delete; CI unchanged (e2e gates develop, main ungated per AD-16, revisit at v1.0.0).
