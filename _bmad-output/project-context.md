...
10. **Jules delegation rule (decision 2026-07-21):** User stories (user-visible, multi-file, need product context) are executed by **local agents** via the BMAD story workflow. Small self-contained, verifiable items (tech debt, test gaps, bounded refactors, docs/dead-code) are delegated to the **Google Jules agent** as copy-paste task blocks from `implementation-artifacts/jules-queue.md` — each block is self-contained (Jules has no BMAD skills or project memory), produces exactly one PR to `develop`, and is **never auto-merged** (human review gate). Every Jules task starts with a "verify it still exists" step because `deferred-work.md` may contain stale entries. Architectural work (Runner.py LangGraph migration, Recharts v3, SSE subscription consolidation) and auth-blocked items stay out of the Jules queue.
