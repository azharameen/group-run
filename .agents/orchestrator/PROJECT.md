...
| 2 | Zero Direct File Mutators | Refactor `sprint-status-updater.ts` & `deferred-work-resolver.ts` to remove programmatic writes | M1 | IN_PROGRESS |
| 3 | Skill Manifests & `bmad-help` | Dynamic skill discovery via `.agent/skills/`, `bmad-help.csv` & `/bmad-help` driver calls | M1 | PLANNED |
| 4 | TUI Loop & Interactive Modals | Stream throttling, ANSI safety, `QueryModal` & `EscalationModal` interactive pause/resume | M1 | PLANNED |
