...
**Context**: The browser, hybrid, and future native clients require one identity contract.
Native `EventSource` cannot attach authentication headers, and client-generated profile identity
cannot be trusted.

**Decision**: Use Google-only Firebase Authentication. Clients send short-lived Firebase ID
tokens as Bearer tokens; FastAPI verifies every `/api` request except health, readiness, and CORS
preflight, including authenticated Fetch/Web Streams SSE. `POST /api/auth/bootstrap` atomically
creates or refreshes `users/{uid}` from verified claims. Firestore Security Rules allow only the
owner to read and update allowlisted mutable profile fields. All authenticated users currently
have equal platform access.
...
- Negative: browser Bearer tokens increase XSS sensitivity, mitigated through restrictive CSP,
  dependency auditing, token redaction, and avoiding unsafe HTML
