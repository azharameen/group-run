...
1. **Request-Scoped Streaming (`POST /api/chat/stream`, `POST /api/threads/{thread_id}/stream`)**:
   - Transports agent reasoning, state updates, errors, and task updates for a specific user prompt.
   - Powered by FastAPI `StreamingResponse` wrapping `astream` v2 or `ainvoke`.
   - Consumed via `fetch` with `ReadableStream` reader in [`frontend/src/api/threads.ts`](../../frontend/src/api/threads.ts).

2. **Global Event Bus Streaming (`GET /api/sse`)**:
   - Transports asynchronous system-wide events (`agent.progress`, `idea.created`, `idea.scored`, `interrupt.created`, `interrupt.approved`, `interrupt.rejected`).
   - Powered by `StreamBus` broadcast bus in [`backend/app/infrastructure/events/stream_bus.py`](../../backend/app/infrastructure/events/stream_bus.py).
   - Consumed via browser native `EventSource` in [`frontend/src/api/threads.ts`](../../frontend/src/api/threads.ts).
...
- **Implementation**: [`backend/app/infrastructure/events/stream_bus.py`](../../backend/app/infrastructure/events/stream_bus.py) (`StreamBus.subscribe`, `StreamBus.publish`)
- **Behavior**:
...
- **Implementation**: [`frontend/src/api/threads.ts`](../../frontend/src/api/threads.ts) (`connectSSE`), [`frontend/src/hooks/useChatStream.ts`](../../frontend/src/hooks/useChatStream.ts) (`useEffect` hook)
- **Behavior**:
...
- **Implementation**: [`frontend/src/hooks/useChatStream.ts`](../../frontend/src/hooks/useChatStream.ts) (`handleStopGeneration`, `useEffect` on `activeThreadId`)
- **Behavior**:
...
- **Implementation**: [`frontend/src/api/threads.ts`](../../frontend/src/api/threads.ts) (`streamChat`, `streamThreadMessage`)
- **Behavior**:
...
- **Implementation**: [`backend/app/api/routes/chat.py`](../../backend/app/api/routes/chat.py) (`_chat_stream_generator`), [`backend/app/api/routes/threads.py`](../../backend/app/api/routes/threads.py) (`_thread_stream_generator`), [`frontend/src/hooks/useChatStream.ts`](../../frontend/src/hooks/useChatStream.ts) (`executeSend`)
...
2. **`tasks_update`**: Emitted when agent execution updates task execution progress or status counts.
3. **`interrupt`**: Emitted when an agent requires human approval before proceeding with tool execution.
4. **`error`**: Emitted when processing fails or graph execution raises an exception.
5. **`done`**: **Guaranteed Terminal Event**. Always emitted as the final frame.
...
- **Implementation**: [`backend/app/api/routes/chat.py`](../../backend/app/api/routes/chat.py) (`_error_shape`, `_chat_stream_generator`), [`backend/app/api/routes/threads.py`](../../backend/app/api/routes/threads.py) (`_thread_stream_generator`), [`frontend/src/hooks/useChatStream.ts`](../../frontend/src/hooks/useChatStream.ts) (`executeSend`)
...
- **Implementation**: [`backend/app/api/routes/interrupts.py`](../../backend/app/api/routes/interrupts.py), [`backend/app/services/interrupt_service.py`](../../backend/app/services/interrupt_service.py), [`frontend/src/hooks/useChatStream.ts`](../../frontend/src/hooks/useChatStream.ts)
...
2. **Stream Interception**: The active chat stream yields an `interrupt` stream event. `useChatStream` sets `pendingInterrupt` and appends a visual system approval notification to `rawMessages`.
3. **Deduplication (`activeInterruptIdRef`)**:
   - To prevent duplicate approval notifications when both the stream reader and `StreamBus` deliver the same interrupt event, `useChatStream` maintains `activeInterruptIdRef`.
   - If an incoming interrupt event ID matches `activeInterruptIdRef.current`, it is skipped.
4. **Reconnection Sync**:
   - If the network drops while an interrupt is active, the frontend `EventSource` reconnect handler calls `GET /api/interrupts/pending`.
   - The latest pending interrupt is re-restored to `pendingInterrupt` and `activeInterruptIdRef`, ensuring the user approval dialog remains visible and actionable.
5. **Resolution**: When approved (`PATCH /api/interrupts/{id}/approve`) or rejected (`PATCH /api/interrupts/{id}/reject`), `interrupt.approved` or `interrupt.rejected` events clear `pendingInterrupt` and reset `activeInterruptIdRef` to `null`.
