...
- The Firebase JavaScript SDK manages browser persistence and automatic ID-token refresh; refresh
  tokens are never sent to FastAPI.
- FastAPI verifies the Bearer token before handlers run and attaches a typed principal. One
  forced token refresh/retry is allowed after a 401; a repeated 401 signs the user out.
- `POST /api/auth/bootstrap` derives identity only from verified claims and atomically provisions
  `users/{uid}` in Firestore.
- SSE uses authenticated Fetch/Web Streams and reconnects after token rotation.
- Cloud Run uses Application Default Credentials. Local and CI environments use Firebase
  emulators or developer ADC; service-account JSON is never committed.
