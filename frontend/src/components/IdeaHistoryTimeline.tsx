...
export function IdeaHistoryTimeline({ detail, files = [] }: Props) {
	...
	// Agent discussion items from actual idea data fields
	for (const [field, meta] of Object.entries(AGENT_FIELD_MAP)) {
		const data = idea[field];
		if (!data) continue;
		items.push({
			id: `disc-${field}`,
			type: "discussion",
			timestamp: idea.updated_at || idea.created_at || new Date().toISOString(),
			title: meta.title,
			agentName: meta.agentName,
			agentRole: meta.agentRole,
			answer: typeof data === "string" ? data : JSON.stringify(data, null, 2),
			explanation: `${meta.agentRole} completed analysis for this idea.`,
		});
	}
			...
			<CardHeader className="p-4 pb-3">
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
					...
					<div className="flex items-center gap-1.5 flex-wrap">
						...
						>
							Agent ({items.filter((i) => i.type === "discussion").length})
						</Badge>
						<Badge
